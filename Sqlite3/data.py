...
def run_data(user_input):
    ...
    if country_input == "1":
        data = tracker_data.get_data()
        print(data)
        connect.insert_country_data(data)
        connect.insert_total_data(data)
    ...
    if country_input == "4":
        pprint.pprint(graph_result)
        #if len(graph_result) == 4:

            #create_table = createTable(graph_result)
            #create_graph = CreateGraph(plt, user_input, graph_result)
